...
class Validator 
{
    ...
    {
        if (empty($data)) {
            YCore::exception(STATUS_SERVER_ERROR, 'The $data parameter can\'t be empty');
        }
        if (empty($rules)) {
            YCore::exception(STATUS_SERVER_ERROR, 'The $rules parameter can\'t be empty');
        }
        foreach ($rules as $name => $rule) {
            if (!array_key_exists($name, $data)) {
                YCore::exception(STATUS_SERVER_ERROR, "The {$name} value does not exist");
            }
            ...
            foreach ($arr_rule as $index => $ruleItem) {
                ...
                switch ($ruleName) {
                    ...
                    case 'date' :
                        ...
                        break;
                    default :
                        YCore::exception(STATUS_SERVER_ERROR, 'In the name of the validator illegally');
                        break;
                }
            }
        }
